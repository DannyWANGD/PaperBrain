...
const DEFAULT_CATEGORIES = ["cs.RO", "cs.AI", "cs.LG", "cs.CV"];
...
const RAW_GROUPS = {
  "Computer Science": [
    ...
    "cs.GT|Computer Science and Game Theory", "cs.HC|Human-Computer Interaction", "cs.IR|Information Retrieval",
    "cs.IT|Information Theory", "cs.LG|Machine Learning", "cs.LO|Logic in Computer Science",
    ...
    "cs.PL|Programming Languages", "cs.RO|Robotics", "cs.SC|Symbolic Computation", "cs.SD|Sound",
  ...
  Mathematics: [
    ...
    "math.LO|Logic", "math.MG|Metric Geometry", "math.MP|Mathematical Physics", "math.NA|Numerical Analysis",
  ...
  Physics: [
    ...
    "cond-mat.supr-con|Superconductivity", "gr-qc|General Relativity and Quantum Cosmology",
    ...
    "physics.pop-ph|Popular Physics", "physics.soc-ph|Physics and Society", "physics.space-ph|Space Physics",
...
const ARXIV_CATEGORY_GROUPS = Object.entries(RAW_GROUPS).map(([group, values]) => ({
  group,
...
const ARXIV_CATEGORY_IDS = new Set(
  ARXIV_CATEGORY_GROUPS.flatMap((group) => group.categories.map((category) => category.id)),
);

function filterCategoryGroups(query) {
  const needle = String(query || "").trim().toLocaleLowerCase();
  if (!needle) return ARXIV_CATEGORY_GROUPS;
  return ARXIV_CATEGORY_GROUPS
    .map((group) => ({
      ...group,
      categories: group.categories.filter((category) => (
        `${category.id} ${category.name}`.toLocaleLowerCase().includes(needle)
      )),
    }))
    .filter((group) => group.categories.length);
}

function updateCategoryGroupSelection(selectedValues, groupName, checked) {
  const selected = new Set(selectedValues || []);
  const group = ARXIV_CATEGORY_GROUPS.find((item) => item.group === groupName);
  if (!group) return selected;
  for (const category of group.categories) {
    if (checked) selected.add(category.id);
