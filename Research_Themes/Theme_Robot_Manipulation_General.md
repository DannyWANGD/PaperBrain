...
```mermaid
graph LR
  VLA[VisionLanguageAction] --> CE[CrossEmbodiment]
  WM[WorldModel] --> ZS[ZeroShotPolicy]
  DM[DiffusionPolicy] --> LH[LongHorizonPlanning]
  RL[ReinforcementLearning] --> SR[Sim2RealGap]
  CE --> WM
  ZS --> DM
  LH --> RL
  SR --> VLA
  3DGS[3DRepresentation] --> VLA
```
