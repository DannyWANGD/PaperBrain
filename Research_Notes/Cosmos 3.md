...
```mermaid
graph LR
    A["Language, image, video, audio, action inputs"] --> B["Modality-specific encoders"]
    B --> C["Token packing: AR prefix + DM subsequence"]
    C --> D["Mixture-of-Transformers (dual-tower layers)"]
    D --> E["AR next-token prediction (language, reasoning)"]
    D --> F["Diffusion denoising (visual, audio, actions)"]
    G["World Action Models are Zero-shot Policies"] -.-> C
    H["Beyond Language Modeling (Transfusion)"] -.-> D
    I["Trinity of Consistency Principles"] -.-> D
```
