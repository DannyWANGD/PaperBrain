...
2. [[World Action Models are Zero shot Policies]]: Both works target fast, generalizable action generation for zero-shot robotic deployment. MVP's single-step action generation resolves a key limitation of world action models, which rely on iterative sampling of action sequences that introduces latency incompatible with real-time closed-loop control. Integrating MVP into world action model pipelines enables 100Hz+ control rates for zero-shot task execution.
