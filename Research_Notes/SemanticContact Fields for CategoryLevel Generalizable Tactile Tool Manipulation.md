...
4. [[World Action Models are Zero shot Policies]]: The diffusion policy conditioned on SCFields directly supports the findings of this line of work: the grounded SCFields representation acts as a structured world model observation that enables zero-shot policy transfer to unseen tool instances, without retraining the policy network for new tool geometries.
